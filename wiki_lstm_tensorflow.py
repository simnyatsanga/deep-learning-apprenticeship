...
# Training, validation and test data sets were made using the 70/20/10 split of the original wiki_en.txt file
# First get the line count of wiki_en.txt = 14896
# Then split 70/20/10 lines and use sed UNIX command to get the ranges in the file.
# wiki_en_train.txt -> sed -n '1,10427p;10428q' wiki_en.txt > wiki_en_train.txt
# wiki_en_valid.txt -> sed -n '10427,13407p;13408q' wiki_en.txt > wiki_en_valid.txt
...
def build_vocab(filename):
    ...
    counter = collections.Counter(data) # returns dict of {word : frequency} for every word
    count_pairs = sorted(counter.items(), key=lambda x: (-x[1], x[0])) # returns list of (work : frequency) tuples sorted in DESC order
    words, _ = list(zip(*count_pairs)) # extracts only the words from the tuples
    word_to_id = dict(zip(words, range(len(words)))) # returns {word : x} for x in range of the total words number
...
def load_data():
    # get the data paths
    train_path = os.path.join(data_path, "wiki_en_train.txt")
    valid_path = os.path.join(data_path, "wiki_en_valid.txt")
    test_path = os.path.join(data_path, "wiki_en_test.txt")
...
if args.run_opt == 1:
    ...
    trained_model = args.data_path + "/two-layer-lstm-medium-config-60-epoch-0p93-lr-decay-10-max-lr-59"
